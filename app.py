...
/* Hide the chrome, but NOT the header element itself: when the sidebar is
   collapsed, Streamlit renders the reopen control inside that header. Hiding
   the whole header therefore made a closed sidebar impossible to reopen. */
...
/* Keep the header AND toolbar in the tree: the button that reopens a collapsed
   sidebar (stExpandSidebarButton) is rendered inside the toolbar, so hiding
   either one stranded the user with no way back. Hide the Deploy/menu actions
   only, and let the expand button through. */
header[data-testid="stHeader"]{background:transparent!important; box-shadow:none!important;}
[data-testid="stToolbar"]{background:transparent!important;}
[data-testid="stExpandSidebarButton"]{display:flex!important; z-index:1000;}
/* NOTE: stExpandSidebarButton IS the button element, not a wrapper around one */
button[data-testid="stExpandSidebarButton"],
[data-testid="stSidebarCollapseButton"] button{
  color:var(--green)!important; background:rgba(15,46,31,.92)!important;
  border:1px solid var(--line)!important; border-radius:10px!important;
  transition:border-color .18s, box-shadow .18s, transform .18s;}
button[data-testid="stExpandSidebarButton"]:hover,
[data-testid="stSidebarCollapseButton"] button:hover{
  border-color:var(--green)!important; transform:translateX(2px);
  box-shadow:0 0 18px -6px var(--green-glow)!important;}
...
.sb-note{margin-top:1.6rem; padding-top:1.1rem; border-top:1px solid var(--line-soft);
  color:var(--ink-3); font-size:.82rem; line-height:1.6;}
.sb-h{color:var(--ink-3); font-size:.72rem; letter-spacing:.14em; text-transform:uppercase;
  margin:1.7rem 0 .7rem;}
.topics{display:flex; flex-wrap:wrap; gap:.4rem;}
.topic{background:rgba(63,191,127,.08); border:1px solid var(--line-soft); color:var(--ink-2);
  border-radius:8px; padding:.3rem .6rem; font-size:.83rem; transition:border-color .18s, color .18s;}
.topic:hover{border-color:var(--green-deep); color:var(--ink);}
[data-testid="stSidebar"] .stButton button{min-height:auto; padding:.55rem .9rem;
  border-radius:10px; font-size:.9rem; text-align:center!important;}
...
# Which everyday topics the corpus actually covers, mapped from the ministries
# recorded in data/registry.csv. Derived rather than hardcoded, so the list
# cannot drift out of step with the documents as the corpus grows.
MINISTRY_TOPICS = {
    "Agriculture": "Farming & crop support",
    "Rural Development": "Rural housing",
    "Health": "Health cover",
    "Education": "School scholarships",
    "Tribal Affairs": "ST scholarships",
    "Social Justice": "SC scholarships & pensions",
    "Women and Child": "Maternity & child benefits",
    "Finance": "Banking, loans & insurance",
    "Petroleum": "LPG connections",
}


@st.cache_data
def corpus_topics() -> list[str]:
    registry = ROOT / "data" / "registry.csv"
    if not registry.exists():
        return []
    blob = registry.read_text()
    found = [label for key, label in MINISTRY_TOPICS.items() if key in blob]
...
with st.sidebar:
    ...
    if topics:
        st.markdown('<div class="sb-h">What I can help with</div><div class="topics">'
                    + "".join(f'<span class="topic">{html.escape(x)}</span>' for x in topics)
                    + '</div>', unsafe_allow_html=True)

    st.markdown('<div class="sb-h">Asking well</div>'
                '<p class="sb-text" style="font-size:.88rem">Name the scheme if you know '
                'it, and say who it is for — age, work, state. Ask in whichever language '
                'you are comfortable with; the answer comes back in that language.</p>',
                unsafe_allow_html=True)
    ...
    st.markdown('<p class="sb-note">Answers are generated by AI and are not professional '
